...
opt.saved_model = 'saved_models/cyrillic_g2/cyrillic_g2.pth'
opt.FT = True
#to match number of classes with pretrained model:
def ordered_union(s1, s2):
    return s1 + "".join([char for char in s2 if char not in s1])
opt.lang_char += 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzЂђЃѓЄєЇїЈјЉљЊњЋӢӣӨөӮӯӢӣӨөӮӯӯ'
print(len(opt.lang_char)+len(opt.number)+len(opt.symbol))
print(opt.character)
opt.character = ordered_union(opt.character, opt.lang_char) +'ӮӯӢ'
print(opt.character)
...
def train(opt, show_number = 2, amp=False):
    """ dataset preparation """
    print('I came to training')
    if not opt.data_filtering_off:
        print(opt.data_filtering_off)
